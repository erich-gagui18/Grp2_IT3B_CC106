    ...
    // ⭐️ Import your helper class (This path MUST match the location of your helper file)
    ...
    public class SharedAdminAppointmentViewModel extends AndroidViewModel {
        ...
        public void confirmAppointmentWithDownPayment(int reservationId, double downPaymentAmount) {
            executor.execute(() -> {
                boolean isSuccess = false; // ⭐️ Track success
                try (Connection conn = new ConnectionClass().CONN()) {
                    ...
                    try (PreparedStatement stmt = conn.prepareStatement(query)) {
                        ...
                        Log.d("SharedAdminApptVM", "Confirm Payment Query: " + updated + " rows affected.");
                        if (updated > 0) {
                            isSuccess = true;
                        }
                    }
                ...
                // ⭐️ Run this logic outside the 'try-with-resources' block
                if (isSuccess) {
                    ...
                    Log.d("SharedAdminApptVM", "Calling sendNotificationForStatus for Confirmed");
        ...
        public void updateAppointmentStatus(int reservationId, String newStatus) {
            executor.execute(() -> {
                ...
                try {
                    ...
                    Log.d("SharedAdminApptVM", "Update Status Query ("+newStatus+"): " + updated + " rows affected.");
                ...
                // ⭐️ Run this logic outside the 'try' block
                if (isSuccess) {
                    // Refresh the list to show the status change
                    fetchAppointmentsForDate(lastFetchedDateInMillis);
                    ...
                    if (newStatus.equalsIgnoreCase("Completed") || newStatus.equalsIgnoreCase("Cancelled")) {
                        Log.d("SharedAdminApptVM", "Calling sendNotificationForStatus for " + newStatus);
        ...
        private void sendNotificationForStatus(int reservationId, String status) {
            ...
            try (Connection conn = new ConnectionClass().CONN();
                ...
                if (rs.next()) {
                    ...
                    if (services == null) services = "your appointment"; // Fallback
                    ...
                    Log.d("SharedAdminApptVM", "SENDING NOTIFICATION: " + title);
